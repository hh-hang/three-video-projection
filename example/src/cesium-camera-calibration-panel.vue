        ...
        >
            ...
            >
                <div>
                    ...
                    <span>Cesium.js 2D–3D correspondence solve</span>
                </div>
...
import type { CesiumProjectorTool } from "../../src/cesium-video-projection";
import {
    solveCameraCalibration,
    type CameraCalibrationObservation,
    type CameraCalibrationParameters,
    type CameraCalibrationResult,
} from "../../src/cesium-camera-calibration";

const props = defineProps<{
    /** 用于抓取当前视频帧的 video 元素 */
    videoEl: HTMLVideoElement;
    /** Cesium Viewer，用于在场景中拾取三维点 */
...
// 复用计算对象，避免每帧分配
const pickWindowPosition = new Cesium.Cartesian2();
const markerWorldPoint = new Cesium.Cartesian3();
...
const getCurrentParameters = (): CameraCalibrationParameters => {
    const [longitude, latitude, altitude] = props.projector.cameraPosition;
    return {
        position: [longitude, latitude, altitude],
        azimuthDeg: props.projector.azimuthDeg,
        elevationDeg: props.projector.elevationDeg,
        rollDeg: props.projector.rollDeg,
        fovDeg: props.projector.fov,
        aspect: props.projector.aspect,
        near: props.projector.near,
        far: props.projector.far,
    };
};

/** 将标定参数写回投影相机，并通知父组件刷新控制面板 */
const applyParameters = (parameters: CameraCalibrationParameters) => {
    props.projector.cameraPosition = [...parameters.position];
    ...
    props.projector.fov = parameters.fovDeg;
    props.projector.aspect = parameters.aspect;
    props.projector.near = parameters.near;
    props.projector.far = parameters.far;
...
const getPickScreenError = (
    ...
    if (!Cesium.defined(projected)) return Number.POSITIVE_INFINITY;
    return Math.hypot(projected.x - windowPosition.x, projected.y - windowPosition.y);
};
...
const addSceneMarker = (id: number, world: [number, number, number]) => {
    sceneMarkers.value.push({
        ...
        screenY: 0,
        visible: false,
        title: `ECEF XYZ: ${world.map((value) => value.toFixed(3)).join(", ")}`,
    });
};
...
const pickWorldPoint = (event: MouseEvent) => {
    ...
    const canvas = props.viewer.scene.canvas;
    const rect = canvas.getBoundingClientRect();
    pickWindowPosition.x = event.clientX - rect.left;
    pickWindowPosition.y = event.clientY - rect.top;
    const hit = props.viewer.scene.pickPosition(pickWindowPosition);
    ...
    // 回投偏差超过 5px 时认为拾取不可靠
    const screenErrorPx = getPickScreenError(hit, pickWindowPosition);
...
const updateSceneMarkerPositions = () => {
    ...
    for (const marker of sceneMarkers.value) {
        markerWorldPoint.x = marker.world[0];
        markerWorldPoint.y = marker.world[1];
        markerWorldPoint.z = marker.world[2];
        const projected = Cesium.SceneTransforms.worldToWindowCoordinates(
            props.viewer.scene,
            markerWorldPoint,
        );
        if (!Cesium.defined(projected)) {
        ...
        marker.screenX = rect.left + projected.x;
        marker.screenY = rect.top + projected.y;
    }
...
onMounted(() => {
    ...
    const canvas = props.viewer.scene.canvas;
    // 捕获阶段监听，便于在其他控件之前完成三维点拾取，并过滤相机拖拽松手
